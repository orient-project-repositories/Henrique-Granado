...
class System:  # this is where all the pieces of the ey esystem will be put together an dlinked
    def __init__(self):
        # Objects
        self.eye = Eye()  # create the eye
        self.eye.change_pos(0, 0, 0)
        self.eye.change_scale(0.0753076410465764)

        # Superior and Inferior Rectus
        self.motor1 = Motor()  # create the motor that will have the superior rectus and inferior rectus attached to the eye
        self.motor1.change_scale(0.02, 0.1, 0.01)
        self.motor1.change_pos(0.436, 0, -0.0835)

        sr_1 = np.array([0, 90])  # superior rectus longitude and latitude at the  motor
        sr_2 = np.array([82.47898517199053, 44.75247673069133])  # superior rectus longitude and latitude at the eye
        self.superior_rectus = Elastic(self.motor1, sr_1, self.eye, sr_2)  # creating superior rectus
        self.superior_rectus.change_color([255, 0, 0], [255, 255, 0])
        self.superior_rectus.add_midpoint(np.array([0.2055, 0.0525, 0]))

        ir_1 = np.array([0, -90])  # inferior rectus longitude and latitude at the  motor
        ir_2 = np.array([82.47898517199053, -44.75247673069133])  # inferior rectus longitude and latitude at the eye
        self.inferior_rectus = Elastic(self.motor1, ir_1, self.eye, ir_2)  # creating inferior rectus
        ...
        for muscle in self.muscle_list:
            self.eye.add_parent(muscle)  # adding the muscles/elastics as parents to the eye so they the ycan iinfluence its movement
    ...
    def get_muscle_forces_magnitudes(self):
        aux = {"SR": self.superior_rectus.get_force_magnitude(), "IR": self.inferior_rectus.get_force_magnitude(),
               "MR": self.medial_rectus.get_force_magnitude(), "LR": self.lateral_rectus.get_force_magnitude(),
               "SO": self.superior_oblique.get_force_magnitude(), "IO": self.inferior_oblique.get_force_magnitude()}
        return aux
...
class Elastic:
    def __init__(self, obj1, loc1, obj2, loc2):
        ...
        self.obj1 = obj1  # the starting object that it is attached to
        self.loc1 = loc1  # its longitude and latitude
        self.obj2 = obj2  # the ending object that it is attached to
        self.loc2 = loc2  # its longitude and latitude

        self.start = None  # the coordinates of the start of the elastic in world coordinates
        self.midpoints = [] # its midpoints in world coordinates
        self.end = None # the coordinates of the end of the elastic in world coordinates

        self.start_color = np.array([0, 255, 0])  # color of the elastic at the start (R,G,B)
        self.end_color = np.array([0, 255, 0])  # color of the elastic at the end

        # self.batch = pyglet.graphics.Batch()
        self.update(0)  # an update to define get the values of the start and end points
    ...
    def do_draw(self):  # draws the elastic
        # self.batch = pyglet.graphics.Batch()
    ...
    def get_insertion2_loc(self):  # gets the insertion longitude and latitude at the end point
        return self.loc2
...
class Motor:  # a sphere stretched different amounts in different directions
    def __init__(self):
        # self.image = pic_name
...
class Eye: #
    def get_tex(self,file): # convert image to texture group to draw image on mesh
    ...
    def __init__(self):
        ...
        self.texture_name = 'tex/Eye.png' #image of the eye
        self.batch = pyglet.graphics.Batch()
    ...
    def do_batch(self): # create batch of the mesh of the eye -> a sphere with an image of a pupil drawn on its surface
        tex = self.get_tex(self.texture_name) # uploading image
        self.batch = pyglet.graphics.Batch()
        step = 10
        for lat in range(-90, 90, step): # building the vertex groups of the sphere mesh
            verts = []
            texc = []
            for lon in range(-180, 181, step):
                x = -self.scale[0] * cos(radians(lat)) * cos(radians(lon))
                ...
                s = (lon + 180) / 360.0
                t = ((lat + step) + 90) / 180.0
                verts += [x, y, z] # teh coordinate of the vertexes
                texc += [s, t] # coordinates of the texture image

            self.batch.add(len(verts)//3, GL_TRIANGLE_STRIP, tex, ('v3f', verts), ('t2f', texc))

        points = list(np.add(-self.scale,np.array([0.1, 1, 1])*self.scale)) # the start of the line that will be drawn to demonstrate the looking direction
        points += list(np.add(-self.scale,np.array([-0.5,1,1])*self.scale)) # the end of the line that will be drawn to demonstrate the looking direction
    ...
    def get_point(self, rot): # get the local coordinates of a point at a certain latitude and longitude on the surface of the eye
        long = radians(rot[0])
        lat = radians(rot[1])
        point = self.scale * [-cos(lat) * cos(long), sin(lat), cos(lat) * sin(long)]
        return quaternion.rotate_vectors(self.rot, point)

    def get_current_inertia_tensor(self): # given the orientation of the eye
    ...
    def get_gravity_torque(self): # for the given orietaion of the eye
        rotation_matrix = quaternion.as_rotation_matrix(self.rot)
    ...
    def add_parent(self, parent): # this will be the elastic objects that will be used to move the eye
    ...
    def update(self, dt):  # updates the position of the eye based on its parents
        torque_elastic = 0
